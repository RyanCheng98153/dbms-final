...
@plan_bp.route('/view_data/plans', methods=['GET'])
def view_plans():
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute("SELECT id, book_id, expired_date, is_complete FROM ReadingPlan")
...
def delete_plan(plan_id):
    ...
    conn.commit()
    conn.close()
    return jsonify({"message": "閱讀計劃已刪除！"}), 200

@plan_bp.route('/update_plan', methods=['PUT'])
def update_plan():
    data = request.get_json()
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE ReadingPlan SET expired_date = ?, is_complete = ? WHERE id = ?",
        (data['expired_date'], int(data.get('is_complete', False)), data['id'])
    )
    conn.commit()
    conn.close()
    return jsonify({"message": "閱讀計劃已更新！"}), 200
